...
<head>
  ...
  <title>CopiGaby — Gestión de Usuarios</title>
...
<body class="font-['Nunito'] bg-[#f0f2f7] text-[#1f2937] min-h-screen flex">
  ...
  <div class="ml-[260px] flex-1 flex flex-col min-h-screen">
    ...
    <main class="p-8 flex-1 animate-fade-up">
      <div class="flex justify-between items-center mb-8">
        <div>
          <h1 class="text-2xl font-[900] text-gray-800">Usuarios del Sistema</h1>
          <p class="text-gray-500 text-sm font-semibold">Administre las cuentas y accesos</p>
        </div>
          ...
          <tbody class="text-gray-700 font-semibold text-sm divide-y">
            <?php if(!empty($usuarios)): foreach ($usuarios as $u): ?>
              <tr class="hover:bg-gray-50/80 transition-colors">
                ...
                <td class="px-6 py-4">
                  <span class="px-3 py-1 rounded-full text-[0.65rem] font-black uppercase <?= $u['estado'] ? 'bg-green-100 text-green-600' : 'bg-red-100 text-red-600' ?>">
            ...
            <?php endforeach; else: ?>
              <tr><td colspan="6" class="px-6 py-10 text-center text-gray-400 font-bold italic">No hay usuarios registrados.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
  ...
  <div id="modalRegister" class="fixed inset-0 z-[150] hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
      ...
      <form action="?url=user&type=register" method="POST" class="relative bg-white shadow-xl rounded-custom w-full max-w-md animate-fade-up overflow-hidden">
        <div class="px-6 py-4 border-b flex justify-between items-center bg-gray-50/50">
          <h3 class="text-xl font-black text-navy-dark">Nuevo Usuario</h3>
        </div>
        <div class="p-6 space-y-4">
           ...
           <div class="flex gap-0">
             <span class="inline-flex items-center px-3 py-2 bg-gray-200 border border-r-0 rounded-l-lg text-sm font-black text-navy-dark">V-</span>
              <input type="tel" inputmode="numeric" pattern="[0-9]*" name="cedula" required class="w-full px-4 py-2 bg-gray-50 border rounded-r-lg focus:border-orange outline-none font-bold no-spinner text-sm" placeholder="12345678">
           </div></div>
          <div><label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Nombre de Usuario <span class="text-red-500">*</span></label>
                     <input type="text" name="nombre_usuario" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold text-sm" placeholder="juan.perez"></div>
           <div><label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Teléfono <span class="text-red-500">*</span></label>
                       <input type="tel" inputmode="numeric" pattern="[0-9]*" name="telefono" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold text-sm" placeholder="04121234567"></div>
          ...
          <div><label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Contraseña <span class="text-red-500">*</span></label>
                     <input type="password" name="password" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold text-sm" placeholder="Mínimo 6 caracteres"></div>
  ...
  <div id="modalEdit" class="fixed inset-0 z-[150] hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
      ...
      <form action="?url=user&type=update" method="POST" class="relative bg-white shadow-xl rounded-custom w-full max-w-md animate-fade-up overflow-hidden">
        <input type="hidden" name="idUser" id="edit_id">
        <div class="px-6 py-4 border-b flex justify-between items-center bg-gray-50/50">
          <h3 class="text-xl font-black text-navy-dark">Editar Usuario</h3>
        </div>
        <div class="p-6 space-y-4">
          <div><label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Cédula</label>
          <div class="flex gap-0">
            <span class="inline-flex items-center px-3 py-2 bg-gray-200 border border-r-0 rounded-l-lg text-sm font-black text-navy-dark">V-</span>
            <input type="tel" inputmode="numeric" pattern="[0-9]*" name="cedula" id="edit_cedula" required class="w-full px-4 py-2 bg-gray-50 border rounded-r-lg focus:border-orange outline-none font-bold no-spinner">
          </div></div>
          ...
          <div><label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Teléfono <span class="text-red-500">*</span></label>
          <input type="tel" inputmode="numeric" pattern="[0-9]*" name="telefono" id="edit_telefono" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold"></div>
          <div><label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Rol</label>
          ...
          <div class="flex items-center gap-2"><input type="checkbox" name="estado" id="edit_estado" class="w-4 h-4 accent-orange"><label class="text-sm font-bold text-navy-dark uppercase">Activo</label></div>
          <div class="flex justify-end gap-3 pt-4">
  ...
  <div id="modalDetalle" class="fixed inset-0 z-[160] hidden overflow-y-auto">
    <div class="flex items-center justify-center min-h-screen p-4">
      ...
      <div class="relative bg-white shadow-2xl rounded-custom w-full max-w-md animate-fade-up overflow-hidden">
        <div class="px-6 py-4 border-b flex justify-between items-center bg-gray-50/50">
          <h3 class="text-xl font-black text-navy-dark">Detalle del Usuario</h3>
        ...
        <div class="p-6 space-y-4">
          ...
          <div><p class="text-[0.65rem] font-black text-gray-400 uppercase mb-1">Usuario</p><p id="det_nombre" class="font-bold text-navy-light uppercase">—</p></div>
          <div><p class="text-[0.65rem] font-black text-gray-400 uppercase mb-1">Teléfono</p><p id="det_telefono" class="font-semibold">—</p></div>
          <div><p class="text-[0.65rem] font-black text-gray-400 uppercase mb-1">Rol</p><p id="det_rol" class="font-bold text-navy">—</p></div>
  ...
  <script>
    ...
    function viewDetails(data) {
      ...
      document.getElementById('btnDetalleEliminar').onclick = () => eliminar(currentRecord.cedula_usuario);
    ...
    function openEditModal(data) {
      document.getElementById('edit_id').value = data.cedula_usuario;
      document.getElementById('edit_cedula').value = data.cedula_usuario;
      document.getElementById('edit_nombre').value = data.nombre_usuario;
      document.getElementById('edit_telefono').value = data.telefono;
      document.getElementById('edit_rol').value = data.codigo_rol;
      document.getElementById('edit_estado').checked = data.estado == 1;
      toggleModal('modalEdit');
    }
    function eliminar(id) {
      if(confirm('¿Desea desactivar este usuario?')) {
        const f = new FormData(); f.append('deleteUser', 'true'); f.append('idUser', id);
        fetch('?url=user&type=main', { method:'POST', body:f }).then(r => r.json()).then(d => {
          if(d.status === 'success') location.reload(); else alert('Error al procesar');
        });
      }
    }
  </script>
