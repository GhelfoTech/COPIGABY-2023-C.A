...
<body class="font-['Nunito'] bg-[#f0f2f7] text-[#1f2937] min-h-screen flex">
        ...
        <table class="w-full text-left">
          <thead>
            <tr class="bg-navy-dark text-white text-[0.7rem] uppercase tracking-widest">
              ...
              <th class="px-6 py-4">Tasa de Cambio</th>
            ...
            <?php if(!empty($monedas)): foreach ($monedas as $m): ?>
              <tr class="hover:bg-gray-50/80 transition-colors">
                ...
                <td class="px-6 py-4 text-center flex justify-center gap-2">
                  <button onclick='openEditModal(<?= json_encode($m) ?>)' class="text-blue-400 hover:text-blue-600 p-2 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"/></svg>
                  </button>
                  <button onclick="eliminar(<?= $m['codigo_moneda'] ?>)" class="text-red-400 hover:text-red-600 p-2 transition-colors">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
                  </button>
                </td>
              </tr>
            <?php endforeach; else: ?>
              <tr><td colspan="6" class="px-6 py-10 text-center text-gray-400 font-bold italic">No hay registros de monedas.</td></tr>
            <?php endif; ?>
          </tbody>
        </table>
      </div>
      ...
      <div class="relative bg-white shadow-xl rounded-custom w-full max-w-md animate-fade-up overflow-hidden">
        ...
        <form action="?url=moneda&type=register" method="POST" class="p-6">
          <div class="mb-4">
            ...
            <input type="text" name="nombre_moneda" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold" placeholder="Ej: Dólar">
          </div>
          <div class="mb-4">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Símbolo</label>
            <input type="text" name="simbolo" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold" placeholder="Ej: $">
          </div>
          <div class="mb-4">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Tasa de Cambio</label>
            <input type="number" step="0.01" name="tasa_cambio" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold">
          </div>
    ...
    <div class="flex items-center justify-center min-h-screen p-4">
      ...
      <div class="relative bg-white shadow-xl rounded-custom w-full max-w-md animate-fade-up overflow-hidden">
        ...
        <form action="?url=moneda&type=update" method="POST" class="p-6">
          ...
          <div class="mb-4">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Nombre</label>
            <input type="text" name="nombre_moneda" id="edit_nombre" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold">
          </div>
          <div class="mb-4">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Símbolo</label>
            <input type="text" name="simbolo" id="edit_simbolo" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold">
          </div>
          <div class="mb-4">
            <label class="block text-xs font-black text-gray-400 uppercase tracking-widest mb-1">Tasa de Cambio</label>
            <input type="number" step="0.01" name="tasa_cambio" id="edit_tasa" required class="w-full px-4 py-2 bg-gray-50 border rounded-lg focus:border-orange outline-none font-bold">
          </div>
  ...
  <script>
    ...
    function eliminar(id) {
      if(confirm('¿Desea desactivar esta moneda?')) {
        const f = new FormData();
        f.append('deleteMoneda', 'true');
        f.append('idmoneda', id);
